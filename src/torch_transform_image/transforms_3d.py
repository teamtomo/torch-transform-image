...
def rotate_then_shift_image_3d(
    ...
    ----------
    image : torch.Tensor
        The 3D image to be shifted/rotated.
    ...
    rotate_z_tensor = torch.as_tensor(rotate_zyx[0], device=image.device, dtype=torch.float32)
    rotate_y_tensor = torch.as_tensor(rotate_zyx[1], device=image.device, dtype=torch.float32)
    rotate_x_tensor = torch.as_tensor(rotate_zyx[2], device=image.device, dtype=torch.float32)
